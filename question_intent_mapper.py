...
try:
    ...
    _ADVANCED_CLUSTERING = True
except Exception:
    _ADVANCED_CLUSTERING = False
    ...
    def cluster_questions(self, questions, n_clusters=None, min_cluster_size=5):
        ...
        if n_clusters:
            ...
            if _ADVANCED_CLUSTERING:
                ...
                # Fallback to KMeans with heuristic k
                k = max(3, min(10, len(questions) // max(1, min_cluster_size)))
                print(f"Advanced clustering unavailable, falling back to KMeans (k={k})...")
                clustering = KMeans(n_clusters=k, random_state=42).fit(embeddings)
                labels = clustering.labels_
