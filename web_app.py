...
class AIOWebApp:
    ...
    def render_visibility_monitor(self):
        ...
        with st.expander("Configuration", expanded=True):
            ...
            with col2:
                ...
                query_mode = st.radio(
                    "Query Mode",
                    ["Auto-generate queries", "Custom prompts"],
                    key="visibility_query_mode"
                )
                
                if query_mode == "Auto-generate queries":
                    ...
                    custom_prompts = st.text_area(
                        ...
                        value="What are the best AI content optimization tools?\nCompare top SEO tools for content optimization\nHow to optimize content for AI visibility?",
                        height=200,
                        help="Enter your custom prompts, one per line. These will be used exactly as written.",
                        key="visibility_custom_prompts"
                    )
                
                # Demo mode toggle to avoid external logins
                use_demo = st.checkbox(
                    "Use demo data (no external logins)",
                    value=True,
                    key="visibility_demo_mode"
                )
                ...
                with st.spinner("Running AI visibility check..."):
                    try:
                        ...
                        if query_mode == "Auto-generate queries":
                            ...
                            # Use custom prompts
                            queries = [p.strip() for p in custom_prompts.split('\n') if p.strip()]
                        
                        # If demo mode is enabled, use simulated results and skip any external logins
                        if 'use_demo' in locals() and use_demo:
                            results = self._generate_demo_visibility_results(
                                brand_name,
                                queries[:num_queries],
                                mapped_platforms
                            )
                        else:
                            # Run visibility check (may require credentials for certain platforms)
                            loop = asyncio.new_event_loop()
                            asyncio.set_event_loop(loop)
                            
                            if "chatgpt" in mapped_platforms:
                                results = loop.run_until_complete(
                                    self._run_chatgpt_visibility_via_simulator(
                                        brand_name,
                                        competitor_list,
                                        queries[:num_queries]
                                    )
        ...
        if st.session_state.visibility_results:
            ...
            if results.get('success', False):
                ...
                with tab2:
                    # Detailed view with full prompts and responses
                    for idx, result in enumerate(results.get('results', [])):
                        with st.expander(f"Query {idx + 1}: {result.get('query', '')[:100]}..."):
                            col1, col2, col3 = st.columns([1, 1, 1])
                            with col1:
                                st.markdown("**Platform**")
                                st.write(result.get('engine_name', result.get('platform', 'unknown')))
                            with col2:
                                st.markdown("**Brand Mentioned**")
                                st.write("✅" if result.get('brand_mentioned') else "❌")
                            with col3:
                            ...
                            st.markdown("**Prompt/Query:**")
                            st.code(result.get('query', ''), language='text')
                            
                            st.markdown("**AI Response:**")
                            st.markdown(result.get('ai_response', 'No response available'))
                            
                            if result.get('sources_cited'):
                                st.markdown("**Sources Cited:**")
                                for source in result.get('sources_cited'):
                                    st.markdown(f"- {source}")
                            
                            if result.get('mention_context'):
                                st.markdown("**Brand Mention Context:**")
                                st.markdown(f"*{result.get('mention_context')}*")
                            
                            st.markdown("**Response Time:** " + f"{result.get('response_time', 0):.2f}s")
        ...
        for platform in platforms:
            ...
            for query in queries:
                ...
                demo_results.append({
                    ...
                    'sentiment': sentiment,
                    'citations': ['example.com', 'techcrunch.com', 'forbes.com'][:random.randint(0, 3)],
    ...
    def render_crawler_analytics(self):
                ...
                if log_file:
                    with st.spinner("Analyzing crawler activity..."):
                        try:
                            ...
                            # Real analysis using AICrawlerAnalytics
                            analysis_result = self.crawler_analytics.parse_access_logs(
                                tmp_file_path,
                                days_back=days_back
                            )

                            if generate_report and analysis_result and not analysis_result.get('error'):
                                report_md = self.crawler_analytics.generate_crawler_report()
                                analysis_result['report_preview'] = report_md[:800]
        ...
        if st.session_state.crawler_results:
            ...
            with col2:
                ai_bots = results.get('ai_requests', 0)
            ...
            with col4:
                unique_bots = len(results.get('crawlers_detected', {}))
            ...
            if 'top_crawled_pages' in results and results['top_crawled_pages'] and 'pages' in results['top_crawled_pages']:
                st.markdown("### Top Pages by AI Bot Visits")
                
                pages_df = pd.DataFrame(results['top_crawled_pages']['pages'])
                fig = px.bar(
                    pages_df.head(10),
                    x='url',
                    y='visits',
            ...
            # AI bots breakdown
            if 'crawlers_detected' in results and results['crawlers_detected']:
                st.markdown("### AI Bot Breakdown")
                
                bots = []
                for bot_type, stats in results['crawlers_detected'].items():
                    bots.append({'bot_name': bot_type, 'requests': stats.get('requests', 0)})
            ...
            if 'recommendations' in results and results['recommendations']:
                ...
                for rec in results['recommendations']:
                    ...
                    st.markdown(f"- **{rec.get('category','Recommendation')}**: {title}")
    ...
    async def _run_chatgpt_visibility_via_simulator(self, brand_name: str, competitors: list, queries: list) -> dict:
        """Run visibility using local Playwright simulator for ChatGPT (real, not demo)."""
        import os
        from datetime import datetime
        simulator = AirtopLLMSimulator()
        await simulator.initialize()
        email = os.getenv('LLM_EMAIL') or os.getenv('CHATGPT_EMAIL')
        password = os.getenv('LLM_PASSWORD') or os.getenv('CHATGPT_PASSWORD')
        if not email or not password:
            await simulator.cleanup()
            return {"success": False, "error": "ChatGPT credentials not found in environment"}

        authed = await simulator.authenticate_platform('chatgpt', {"email": email, "password": password})
        if not authed:
            await simulator.cleanup()
            return {"success": False, "error": "Failed to authenticate with ChatGPT"}

        df = await simulator.check_brand_visibility(
            brand_name=brand_name,
        ...
        for _, row in df.iterrows():
            results.append({
                ...
                'sentiment': 'positive' if row.get('mention_type') in ('direct_positive',) else 'neutral',
                'citations': len(row.get('web_search_results', []) or []),
                'timestamp': (row.get('timestamp') or datetime.now()).isoformat(),
                'success': bool(row.get('success', False)),
                'ai_response': row.get('response', ''),
                'sources_cited': row.get('web_search_results', [])[:3],
